...
// Baris pendek berformat "nama prosedur" berdiri sendiri (tanpa titik dua, tanpa kalimat
// perintah) yang menandai pergantian topik utama DALAM 1 IK yang sama -- misal dokumen
// "Running Test Turbin" yang memuat prosedur "Start Turbin" lalu "Stop Turbin" tanpa ada
// heading eksplisit (":") di antaranya, karena di Word aslinya kemungkinan cuma di-bold,
// bukan format heading yang bisa dibaca lewat teks polos.
//
// Daftar ini SENGAJA sempit (whitelist kata kunci tertentu, bukan "semua baris pendek")
// supaya tidak salah anggap langkah kerja singkat seperti "Klik Release" atau "Tekan
// tombol 5E" sebagai header baru -- itu tetap bagian dari 1 prosedur yang sama, bukan
// pergantian topik. Sudah dites terhadap seluruh chunk existing (Juli 2026): 5 match,
// semuanya header asli, 0 false-positive.
const NAMED_PROCEDURE_HEADER_PATTERNS = [
  /^start\s+\w+$/i,
  /^stop\s+\w+$/i,
  /^normal\s+stop$/i,
  /^quick\s+stop$/i,
...
function looksLikeNamedProcedureHeader(line) {
  const trimmed = line.trim();
  if (!trimmed || trimmed.length > 30) return false;
  if (/[.:,;]$/.test(trimmed)) return false; // kalimat perintah biasanya diakhiri tanda baca -- header nama prosedur tidak
  return NAMED_PROCEDURE_HEADER_PATTERNS.some((re) => re.test(trimmed));
}
...
function splitProcedure(lines) {
  ...
  for (const line of lines) {
    ...
    const isColonHeader = line.replace(/\s+$/, '').endsWith(':') && !isSubstep;
    const isNamedHeader = !isSubstep && !isColonHeader && looksLikeNamedProcedureHeader(line);
    const isHeaderLine = isColonHeader || isNamedHeader;
